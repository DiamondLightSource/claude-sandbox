...
Device passthrough is off by default. `--gpu` and `--device` expose selected
...
GPU resource exhaustion and device side effects are not contained. Raw block
devices can bypass filesystem restrictions.

Enable only the devices the workload needs. See
[GPUs and other devices](../how-to/use-the-container-image.md#gpus-and-other-devices).
