...
| `/usr/libexec/claude-sandbox/version` | Stamped by `install.sh` (`git describe` on the installing clone, or the `CLAUDE_SANDBOX_VERSION` build arg) | What `claude-sandbox version` reports: a release tag when installed from one (e.g. via `claude-sandbox update`), a commit hash for an unpinned main clone |
