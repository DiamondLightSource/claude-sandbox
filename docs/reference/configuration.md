...
| `local-model-host` | `127.0.0.1` (default), `gateway`, or an IPv4 address | Where the outer end of the loopback relay connects, for every relayed port. `gateway` is the container's default next-hop, which user-mode networking (slirp4netns with `allow_host_loopback=true`, or pasta) maps to the host's loopback — the setting for bridge containers, where `127.0.0.1` is the container's own loopback. Hostnames are refused. Environment `CLAUDE_SANDBOX_LOCAL_MODEL_HOST` takes precedence. See [Reach services on the host's loopback](../how-to/network-egress-jail.md#reach-services-on-the-hosts-loopback) |
...
| `CLAUDE_SANDBOX_LOCAL_MODEL_HOST` | you (env, per session) / conf `local-model-host` → shadow | Where the outer relay end connects: `127.0.0.1` (default), `gateway`, or an IPv4 address. Env wins over conf |
