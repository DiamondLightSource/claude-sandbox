...
| `local-model-port` | TCP port 1–65535; `0` disables; bare flag means `1920` | Pi-only, opt-in relay from the outer container's IPv4 loopback to Pi's loopback. Requires the network jail and `socat`. Environment `CLAUDE_SANDBOX_LOCAL_MODEL_PORT` takes precedence. See [Use Pi](../how-to/use-pi.md) for lllm2 and container networking |
