...
| `keep-xtmodkeys` | bare flag / `1` | Equivalent to `CLAUDE_SANDBOX_KEEP_XTMODKEYS=1`: leave xterm modifyOtherKeys sequences in the agent's output. By default the shadow strips them — see the environment table below |
...
| `CLAUDE_SANDBOX_KEEP_XTMODKEYS` | you (env, per session) / conf `keep-xtmodkeys` → shadow | Every agent enables xterm modifyOtherKeys (`ESC[>4;2m`) at startup, and the terminals on RHEL 8/9 desktops draw that sequence as visible junk beside the prompt instead of dropping it. The shadow therefore strips `CSI > Ps;Ps m` from the agent's output by default; nothing else is touched and the kitty keyboard protocol still negotiates normally. Set `1` on a terminal that honours modifyOtherKeys (xterm, recent VTE) to keep the extra key chords it enables (Shift+Enter newline, Ctrl+Shift+letter). Env wins over conf |
