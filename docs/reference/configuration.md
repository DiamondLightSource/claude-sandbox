...
| `local-model-port` | TCP port 1–65535; shipped default `1920`; `0` disables; bare flag means `1920` | Relays one port from the outer container's IPv4 loopback to the same port on the agent's loopback, for every agent ({ref}`adr-local-port-all-agents`); Pi also runs model/context discovery on it at startup. Requires the network jail and `socat`. Environment `CLAUDE_SANDBOX_LOCAL_MODEL_PORT` takes precedence, so a single session can pick a different port. See [Reach a service on the host's loopback](../how-to/network-egress-jail.md#reach-a-service-on-the-hosts-loopback) and [Use Pi](../how-to/use-pi.md) |
