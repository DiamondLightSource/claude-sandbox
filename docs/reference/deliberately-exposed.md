...
| `/root/.agents/skills/` | rw | Skills shared by every agent (Claude, Codex, Pi), in **every** session. Codex and Pi load them natively; Claude via symlinks in `~/.claude/skills`. Symlinked to `/user-terminal-config/.agents/skills` when that is mounted. Any agent can write a skill here that the others then load, so treat it like your own `~/.claude`. The rest of `/root/.agents/` is NOT bound ({ref}`adr-shared-agent-skills`) |
