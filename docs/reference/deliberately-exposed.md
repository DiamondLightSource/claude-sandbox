...
| Configured devices | rw | `gpu` / `allow-device` expose hardware and its driver interface |
...
| GPU exhaustion or device side effects | Limit which hardware the workload can access; raw disks bypass filesystem protections |
