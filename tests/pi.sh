...
unset AGENT_SANDBOX_ALLOW_WRITE AGENT_SANDBOX_PASS_ENV AGENT_SANDBOX_NO_FORGE AGENT_SANDBOX_LOCAL_PORTS AGENT_SANDBOX_CALLBACK_PORTS
...
assert_contains pi-command "$argv" /usr/libexec/agent-sandbox/pi-run
...
for agent in claude codex; do
    ...
    if AGENT_SANDBOX_LOCAL_MODEL_PORT=1920 local_model_enabled; then pass
    else fail "$agent does not enable the local relay"; fi
    argv="$(AGENT_SANDBOX_LOCAL_MODEL_PORT=8082 bwrap_argv_build /repo "$AGENT_REAL")"
    assert_pair "$agent-relay-port" "$argv" AGENT_SANDBOX_LOCAL_MODEL_PORT 8082
done
agent_profile pi
unset AGENT_SANDBOX_LOCAL_MODEL_PORT
if local_model_enabled; then fail 'relay enabled without configuration'; else pass; fi
parse_config "$REPO_ROOT/.devcontainer/agent-sandbox.conf"
assert_eq shipped-relay-port 1920 "$AGENT_SANDBOX_LOCAL_MODEL_PORT"
argv="$(bwrap_argv_build /repo "$AGENT_REAL")"
assert_pair discovery-port "$argv" AGENT_SANDBOX_LOCAL_MODEL_PORT 1920
...
export AGENT_SANDBOX_LOCAL_MODEL_PORT=0
parse_config "$tmp/conf"
...
for port in -1 65536 '1920,fork' localhost:1920 01920 99999999999999999999; do
    if AGENT_SANDBOX_LOCAL_MODEL_PORT="$port" validate_local_model_port 2>/dev/null; then
        fail "accepted invalid relay port: $port"
    else pass; fi
    if AGENT_SANDBOX_LOCAL_MODEL_PORT=0 AGENT_SANDBOX_LOCAL_PORTS="8082 $port" validate_local_model_port 2>/dev/null; then
...
unset AGENT_SANDBOX_LOCAL_MODEL_PORT AGENT_SANDBOX_LOCAL_PORTS
export AGENT_SANDBOX_LOCAL_PORTS='8082,9000 8082'
parse_config "$tmp/conf"
...
argv="$(bwrap_argv_build /repo "$AGENT_REAL")"
assert_not_contains no-discovery-without-model-port "$argv" AGENT_SANDBOX_LOCAL_MODEL_PORT
unset AGENT_SANDBOX_LOCAL_MODEL_PORT AGENT_SANDBOX_LOCAL_PORTS
if AGENT_SANDBOX_LOCAL_MODEL_PORT=0 local_model_enabled; then fail 'relay enabled with nothing configured'; else pass; fi
...
unset AGENT_SANDBOX_LOCAL_MODEL_PORT AGENT_SANDBOX_LOCAL_PORTS AGENT_SANDBOX_CALLBACK_PORTS
parse_config "$REPO_ROOT/.devcontainer/agent-sandbox.conf"
...
export AGENT_SANDBOX_CALLBACK_PORTS='1456,1455'
parse_config "$tmp/conf"
assert_eq callback-set $'1456\n1455\n53692' "$(callback_ports)"
assert_parse callback-set-valid validate_callback_ports
unset AGENT_SANDBOX_CALLBACK_PORTS
for port in -1 65536 '1455,fork' localhost:1455 01455 99999999999999999999; do
    if AGENT_SANDBOX_CALLBACK_PORTS="$port" validate_callback_ports 2>/dev/null; then
        fail "accepted invalid callback-port entry: $port"
    else pass; fi
done
if AGENT_SANDBOX_LOCAL_MODEL_PORT=1920 AGENT_SANDBOX_CALLBACK_PORTS=1920 validate_callback_ports 2>/dev/null; then
    fail 'accepted the model port as a callback-port'
else pass; fi
if AGENT_SANDBOX_LOCAL_MODEL_PORT=0 AGENT_SANDBOX_LOCAL_PORTS=8082 AGENT_SANDBOX_CALLBACK_PORTS='53692 8082' validate_callback_ports 2>/dev/null; then
    fail 'accepted a local-port as a callback-port'
else pass; fi

cli="$REPO_ROOT/.devcontainer/agent-sandbox/agent-sandbox"
...
rc=0
env -u IS_SANDBOX -u IS_SANDBOX_AGENT bash "$REPO_ROOT/.devcontainer/agent-sandbox/pi-run" >/dev/null 2>&1 || rc=$?
assert_eq unwrapped-pi-blocked 2 "$rc"
rc=0
IS_SANDBOX=1 IS_SANDBOX_AGENT=claude bash "$REPO_ROOT/.devcontainer/agent-sandbox/pi-run" >/dev/null 2>&1 || rc=$?
...
(
    export AGENT_SANDBOX_SMOKE=1 WITH_PI=0 INSTALL_PREFIX="$tmp/install" INSTALL_USER_HOME="$HOME"
...
expect_file "$tmp/install/usr/libexec/agent-sandbox/pi-run"
expect_file "$tmp/install/usr/libexec/agent-sandbox/pi-system.md"
assert_parse pi-system-note cmp -s "$REPO_ROOT/.devcontainer/agent-sandbox/pi-system.md" "$tmp/install/usr/libexec/agent-sandbox/pi-system.md"
if grep -q "uvx" "$tmp/install/usr/libexec/agent-sandbox/pi-system.md"; then pass; else fail "pi system note does not mention uv"; fi
assert_eq pi-system-note-mode 644 "$(stat -c %a "$tmp/install/usr/libexec/agent-sandbox/pi-system.md")"
...
(
    export AGENT_SANDBOX_SMOKE=0 WITH_PI=1 INSTALL_PREFIX="$tmp/release-install"
    unset PI_VERSION
    source "$REPO_ROOT/.devcontainer/agent-sandbox/install.sh"
    ...
    install_pi_binary
    test -x "$PREFIX/usr/libexec/agent-sandbox/pi-dist/pi"
    test -f "$PREFIX/usr/libexec/agent-sandbox/pi-dist/asset"
    test ! -e "$HOME/.local/bin/pi"
    test "$(cat "$PREFIX/usr/libexec/agent-sandbox/pi-dist/.sandbox-version")" = 0.85.1
    ...
    test "$(cat "$PREFIX/usr/libexec/agent-sandbox/pi-dist/.sandbox-version")" = 0.85.2
