...
probe_userns_or_refuse() {
    ...
    if ! bwrap --ro-bind / / --unshare-user-try --unshare-pid --proc /proc -- /bin/true \
            >/dev/null 2>&1; then
        cat >&2 <<'EOF'
claude-sandbox: refusing — user namespace or fresh procfs setup failed.
The bwrap sandbox requires both.

For rootless Podman, recreate with --security-opt 'unmask=/proc/*'.
Devcontainers need that option in runArgs. NVIDIA devices must use the
sandbox-gpu CDI class generated by container/setup-nvidia-cdi.sh.
