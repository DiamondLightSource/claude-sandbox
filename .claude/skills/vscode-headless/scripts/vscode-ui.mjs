...
if (action === 'help') {
  console.log(`Usage: node vscode-ui.mjs ACTION [ARGS]
...
Set VSCODE_HEADLESS_PORT (default 9222) and, when there is more than one
window, VSCODE_HEADLESS_WINDOW to an exact window ID from 'windows'.
Start VS Code with vscode-headless from this directory (Xvfb plus DevTools
inside the sandbox), or pass that wrapper to any tool that takes a code binary path.
...
let socket;
try {
  const port = Number(process.env.VSCODE_HEADLESS_PORT || 9222);
  ...
  const selected = process.env.VSCODE_HEADLESS_WINDOW;
  const matches = selected ? windows.filter(w => w.id === selected) : windows;
  if (matches.length !== 1) throw Error('Select one window with VSCODE_HEADLESS_WINDOW; run windows first');
